        ...
        <v-sheet>
            <v-card class="ma-3" v-for="item in artList" :key="item.id" link
                ...
                <v-row no-gutters class="d-flex align-center">
                    ...
                    <v-col>
                        <v-card-title>
                            <v-chip color="pink" outlined label class="mr-3 text-white">{{
                                item.Category.name
                            }}</v-chip>
                            <div>{{ item.title }}</div>
                        </v-card-title>
                        ...
                        <v-card-text class="d-flex align-center">
                            <div class="d-flex align-center">
                                ...
                                <span>{{
                                    this.$moment(item.CreatedAt).format('YYYY-MM-DD HH:MM')
                                }}</span>
                            </div>
                        </v-card-text>
                    </v-col>
            ...
            <v-col>
                <div class="text-center">
                    <v-pagination total-visible="7" v-model="queryParam.pagenum"
                        :length="Math.ceil(total / queryParam.pagesize)" @click="getArtList()"></v-pagination>
                </div>
            </v-col>
        </v-sheet>
    </v-container>
